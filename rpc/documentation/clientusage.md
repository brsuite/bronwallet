...
auto main() -> int {
    ...
    if (!status.ok()) {
        ...
        std::cout << "Spendable balance: " << response.spendable() << " Broneess" << std::endl;
    }
}
```
    ...
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Spendable balance: {balanceResponse.Spendable} Broneess");
            }
            finally
            {
...
client.balance(request, function(err, response) {
    if (err) {
        ...
        console.log('Spendable balance:', response.spendable, 'Broneess');
    }
});
```
...
def main():
    ...
    print 'Spendable balance: %d Broneess' % response.spendable
