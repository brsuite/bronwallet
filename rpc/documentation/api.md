...
management of the wallet and its connection to the Brocoin network.  It has no
...
start the brond RPC client.  This RPC client is used for wallet syncing and
publishing transactions to the Brocoin network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default brond RPC port of the
  active Brocoin network.
...
- `int64 spendable`: The spendable balance, given some number of required
  confirmations, counted in Broneess.  This equals the total balance when the
...
- `int64 target_amount`: If positive, the service may limit output results to
  those that sum to at least this amount (counted in Broneess).  If zero, all
...
- `repeated PreviousOutput selected_outputs`: The output set returned as a list
  ...
  **Nested message:** `PreviousOutput`
  ...
  - `int64 amount`: The output value (counted in Broneess) of the unspent
    transaction output.
...
the Brocoin network.  If the transaction spends any of the wallet's unspent
