...
//
//   https://github.com/brsuite/bronwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/brsuite/bronwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
// as well as establishing a RPC connection to a brond consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := bronutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount bronutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += bronutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > bronutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > bronutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to brond could result in the tx
