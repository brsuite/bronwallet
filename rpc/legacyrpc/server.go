...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="bronwallet RPC"`)
...
// method.  This may be a request that is handled directly by bronwallet, or
// a chain server request that is handled by passing the request down to brond.
//
...
func (s *Server) handlerClosure(request *bronjson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *bronjson.Request) string {
...
// but this is only implemented here to work around an oddity with bronjson,
...
func (s *Server) invalidAuth(req *bronjson.Request) bool {
	cmd, err := bronjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*bronjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := bronjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := bronjson.MarshalResponse(req.ID, nil, bronjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	case "stop":
		stop = true
		res = "bronwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(&req)()
	}

	// Marshal and send.
	mresp, err := bronjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
