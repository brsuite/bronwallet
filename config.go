...
import (
	...
	"github.com/brsuite/bronwallet/internal/cfgutil"
	"github.com/brsuite/bronwallet/internal/legacy/keystore"
	"github.com/brsuite/bronwallet/netparams"
...
const (
	defaultCAFilename       = "brond.cert"
	defaultConfigFilename   = "bronwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "bronwallet.log"
...
var (
	brondDefaultCAFile  = filepath.Join(bronutil.AppDataDir("brond", false), "rpc.cert")
	defaultAppDataDir  = bronutil.AppDataDir("bronwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of brond RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with brond"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	BrondUsername     string                  `long:"brondusername" description:"Username for brond authentication"`
	BrondPassword     string                  `long:"brondpassword" default-mask:"-" description:"Password for brond authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and brond authentication (if brondusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and brond authentication (if brondpassword is unset)"`
...
//
// The above results in bronwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		if cfg.DisableClientTLS {
			...
			// If CAFile is unset, choose either the copy or local brond cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local brond and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						brondCertExists, err := cfgutil.FileExists(
							brondDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if brondCertExists {
							cfg.CAFile.Value = brondDefaultCAFile
						}
					}
				}
			}
	...
	// If the brond username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for brond and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for brond and wallet.
	if cfg.BrondUsername == "" {
		cfg.BrondUsername = cfg.Username
	}
	if cfg.BrondPassword == "" {
		cfg.BrondPassword = cfg.Password
	}
