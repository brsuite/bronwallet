...
var (
	walletDataDirectory = bronutil.AppDataDir("bronwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet3              bool                `long:"testnet" description:"Use the test brocoin network (version 3)"`
	SimNet                bool                `long:"simnet" description:"Use the simulation brocoin network"`
...
func init() {
	...
	if opts.TestNet3 && opts.SimNet {
		fatalf("Multiple brocoin networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []bronjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue bronutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]bronutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := bronutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(bronutil.Amount) (bronutil.Amount, []*wire.TxIn, []bronutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	sourceOutputs := make(map[string][]bronjson.ListUnspentResult)
	...
	var totalSwept bronutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := bronutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount bronutil.Amount) bool {
	return amount >= 0 && amount <= bronutil.MaxBronees
}

func parseOutPoint(input *bronjson.ListUnspentResult) (wire.OutPoint, error) {
