...
func fetchMinedBalance(ns walletdb.ReadBucket) (bronutil.Amount, error) {
	...
	return bronutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt bronutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (bronutil.Amount, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (bronutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return bronutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (bronutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return bronutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (bronutil.Amount, error) {
	...
	return bronutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (bronutil.Amount, error) {
	...
	return bronutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = bronutil.Amount(byteOrder.Uint64(it.cv))
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount bronutil.Amount, block *Block, credKey []byte) error {
...
func valueUnminedCredit(amount bronutil.Amount, change bool) []byte {
	v := make([]byte, 9)
	byteOrder.PutUint64(v, uint64(amount))
	if change {
		v[8] = 1 << 1
...
func fetchRawUnminedCreditAmount(v []byte) (bronutil.Amount, error) {
	if len(v) < 9 {
...
func fetchRawUnminedCreditAmountChange(v []byte) (bronutil.Amount, bool, error) {
	if len(v) < 9 {
		str := "short unmined credit value"
		return 0, false, storeError(ErrData, str, nil)
	}
	amt := bronutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
