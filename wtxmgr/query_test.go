// Copyright (c) 2015-2017 The brsuite developers
...
func TestStoreQueries(t *testing.T) {
	...
	newState.blocks[0][0].Credits = []CreditRecord{
		{
			Index:  0,
			Amount: bronutil.Amount(recA.MsgTx.TxOut[0].Value),
			Spent:  false,
			Change: true,
		},
	}
	...
	// amount into outputs of 40 and 60 BRON.
	...
	newState.blocks[0][1].Credits = []CreditRecord{
		{
			Index:  0,
			Amount: bronutil.Amount(recB.MsgTx.TxOut[0].Value),
			Spent:  false,
			Change: false,
		},
	}
