...
var (
	...
	TstRecvTx, _                    = bronutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = bronutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *bronutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := bronutil.NewTxFromBytes(TstRecvSerializedTx)
	// Switch txout amount to 1 BRON.  Transaction store doesn't
	...
	TstDoubleSpendTx := bronutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := bronutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: bronutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: bronutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: bronutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: bronutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: bronutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: bronutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := bronutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 BRON at and beyond
	// maturity.
	//
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance bronutil.Amount,
	...
	checkBalance(bronutil.Amount(initialBalance), false)
	checkBalance(bronutil.Amount(initialBalance), true)
	...
	checkBalance(bronutil.Amount(changeAmount), true)
