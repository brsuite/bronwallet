...
func ExampleStore_Balance() {
	...
	// Insert a transaction which outputs 10 BRON unmined and mark the output
	// as a credit.
	...
	// Output:
	// 10 BRON, 0 BRON, 0 BRON
	// 10 BRON, 10 BRON, 0 BRON
	// 10 BRON, 10 BRON, 10 BRON
}
...
func ExampleStore_Rollback() {
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		// Insert a transaction which outputs 10 BRON in a block at height 100.
...
func Example_basicUsage() {
	...
	// Insert an unmined transaction that outputs 10 BRON to a wallet address
	// at output 0.
	...
	// outputs.  Mark the second one (5 BRON) as wallet change.
