...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32, change bool) (bool, error) {
	if block == nil {
		...
		v := valueUnminedCredit(bronutil.Amount(rec.MsgTx.TxOut[index].Value), change)
	...
	txOutAmt := bronutil.Amount(rec.MsgTx.TxOut[index].Value)
		...
		for i := range b.transactions {
			...
			if blockchain.IsCoinBaseTx(&rec.MsgTx) {
				...
				for i, output := range rec.MsgTx.TxOut {
					...
					if credKey != nil {
						minedBalance -= bronutil.Amount(output.Value)
...
func (s *Store) UnspentOutputs(ns walletdb.ReadBucket) ([]Credit, error) {
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		cred := Credit{
			...
			Amount:       bronutil.Amount(txOut.Value),
...
func (s *Store) Balance(ns walletdb.ReadBucket, minConf int32, syncHeight int32) (bronutil.Amount, error) {
