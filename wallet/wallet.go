...
import (
	...
	"github.com/brsuite/bronwallet/walletdb/migration"
...
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]bronutil.Address,
	[]wtxmgr.Credit, error) {
	...
	err := w.Manager.ForEachDefaultScopeActiveAddress(
		addrmgrNs, func(addr bronutil.Address) error {
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]bronutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]bronutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb bronutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []bronutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (bronutil.Amount, error) {
...
type Balances struct {
	Total          bronutil.Amount
	Spendable      bronutil.Amount
	ImmatureReward bronutil.Amount
}
...
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (bronutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]bronutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a bronutil.Address) (*bronec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a bronutil.Address) (*bronec.PrivateKey, error) {
	var privKey *bronec.PrivateKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		managedAddr, err := w.Manager.Address(addrmgrNs, a)
		if err != nil {
...
func (w *Wallet) HaveAddress(a bronutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a bronutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a bronutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	syncHeight int32, net *chaincfg.Params) []bronjson.ListTransactionsResult {
	...
	results := []bronjson.ListTransactionsResult{}
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal bronutil.Amount
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).ToBRON()
	}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := bronutil.Amount(output.Value).ToBRON()
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]bronjson.ListTransactionsResult, error) {
	txList := []bronjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]bronjson.ListTransactionsResult, error) {
	txList := []bronjson.ListTransactionsResult{}
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]bronjson.ListTransactionsResult, error) {
	txList := []bronjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*bronutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]bronjson.ListTransactionsResult, error) {
	txList := []bronjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*bronutil.Amount)
...
type AccountBalanceResult struct {
	...
	AccountName    string
	AccountBalance bronutil.Amount
}
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	addresses map[string]struct{}) ([]*bronjson.ListUnspentResult, error) {

	var results []*bronjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		results = make([]*bronjson.ListUnspentResult, 0, len(unspent))
		for i := range unspent {
			output := unspent[i]
		...
		scSwitch:
			...
			result := &bronjson.ListUnspentResult{
				TxID:          output.OutPoint.Hash.String(),
				Vout:          output.OutPoint.Index,
				Account:       acctName,
				ScriptPubKey:  hex.EncodeToString(output.PkScript),
				Amount:        output.Amount.ToBRON(),
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr bronutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr bronutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *bronutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]bronutil.Address{addr})
		if err != nil {
...
func (w *Wallet) LockedOutpoints() []bronjson.TransactionInput {
	locked := make([]bronjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = bronjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (bronutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (bronutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32) (bronutil.Address, error) {
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb bronutil.Amount) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*bronutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr bronutil.Address) (*bronec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr bronutil.Address) ([]byte, error) {
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	if rpcErr, ok := err.(*bronjson.RPCError); ok {
		rpcTxConfirmed = rpcErr.Code == bronjson.ErrRPCTxAlreadyInChain
	}
	...
	// brond node that already has it in their mempool.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L953
	...
	// already confirmed to a brond node over the P2P network.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1036
	...
	//
	// This error is returned from brond when there is already a transaction
	// not signaling replacement in the mempool that spends one of the
	// referenced outputs.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L591
	case match(err, "already spent"):
		fallthrough

	// This error is returned from brond when a referenced output cannot be
	// found, meaning it etiher has been spent or doesn't exist.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/blockchain/chain.go#L405
	case match(err, "already been spent"):
		fallthrough

	// This error is returned from brond when a transaction is spending
	// either output that is missing or already spent, and orphans aren't
	// allowed.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1409
	...
	// Returned by brond when replacement transaction was rejected for
	// whatever reason.
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L841
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L854
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L875
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L896
	// https://github.com/brsuite/brond/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L913
...
// in order to allow applications wrapping bronwallet to store app-specific data
