...
func (w *Wallet) PubKeyForAddress(a bronutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a bronutil.Address) (*btcec.PrivateKey, error) {
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := bronutil.Amount(output.Value).ToBTC()
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr bronutil.Address) (*btcec.PrivateKey, bool, error) {
