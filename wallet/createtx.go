...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := bronutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]bronutil.Amount, 0, len(eligible))

	return func(target bronutil.Amount) (bronutil.Amount, []*wire.TxIn,
		[]bronutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr bronutil.Address) (*bronec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr bronutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb bronutil.Amount, dryRun bool) (
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr bronutil.Address
		var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := bronutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []bronutil.Amount) error {
