...
import (
	...
	"github.com/brsuite/bronwallet/wallet/txsizes"
)

// SumOutputValues sums up the list of TxOuts and returns an Amount.
func SumOutputValues(outputs []*wire.TxOut) (totalOutput bronutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += bronutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
// InputSourceError.
type InputSource func(target bronutil.Amount) (total bronutil.Amount, inputs []*wire.TxIn,
	inputValues []bronutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []bronutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb bronutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []bronutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := bronutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
