...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *wtxmgr.TxDetails) TransactionSummary {
	...
	var fee bronutil.Amount
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		for _, deb := range details.Debits {
			fee += deb.Amount
		}
		for _, txOut := range details.MsgTx.TxOut {
			fee -= bronutil.Amount(txOut.Value)
		}
	}
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]bronutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]bronutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]bronutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *wtxmgr.TxDetails) {
	...
	bals := make(map[uint32]bronutil.Amount)
...
type TransactionSummaryInput struct {
	...
	PreviousAmount  bronutil.Amount
}
