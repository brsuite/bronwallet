...
const DefaultRelayFeePerKb bronutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb bronutil.Amount) bronutil.Amount {
	...
	relayFee := bronutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount bronutil.Amount, scriptSize int, relayFeePerKb bronutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb bronutil.Amount) bool {
	...
	return IsDustAmount(bronutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb bronutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > bronutil.MaxBronees {
...
func FeeForSerializeSize(relayFeePerKb bronutil.Amount, txSerializeSize int) bronutil.Amount {
	fee := relayFeePerKb * bronutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > bronutil.MaxBronees {
		fee = bronutil.MaxBronees
	}
