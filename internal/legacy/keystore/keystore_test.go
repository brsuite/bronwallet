...
func TestBronAddressSerializer(t *testing.T) {
	...
	addr, err := newBronAddress(fakeWallet, privKey, nil,
		makeBS(0), true)
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read bronAddress differ.")
	}
}
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// the next pubkeys.
		pubkeyUncompressed, err := bronec.ParsePubKey(nextPubUncompressedFromPub, bronec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := bronec.ParsePubKey(nextPubCompressedFromPub, bronec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
			return
		}
		privkeyUncompressed := &bronec.PrivateKey{
			PublicKey: *pubkeyUncompressed.ToECDSA(),
			D:         new(big.Int).SetBytes(nextPrivUncompressed),
		}
		privkeyCompressed := &bronec.PrivateKey{
		...
		if !ok {
			t.Errorf("%s: bronec signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: bronec signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("bronec signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("bronec signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	pk, _ := bronec.PrivKeyFromBytes(bronec.S256(), make([]byte, 32))
	wif, err := bronutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	pk, err := bronec.NewPrivateKey(bronec.S256())
	if err != nil {
	...
	// import priv key
	wif, err := bronutil.NewWIF((*bronec.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
