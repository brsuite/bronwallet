...
import (
	...
	"github.com/brsuite/bronwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := bronec.PrivKeyFromBytes(bronec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case bronec.PubKeyBytesLenUncompressed, bronec.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case bronec.PubKeyBytesLenUncompressed:
		compressed = false
	case bronec.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := bronec.ParsePubKey(pubkey, bronec.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := bronec.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &bronec.PublicKey{
		Curve: bronec.S256(),
		X:     newX,
		Y:     newY,
	}
...
func getAddressKey(addr bronutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]bronutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]bronutil.Address),
	...
	root, err := newRootBronAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]bronutil.Address)
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		switch bronAddr := wAddr.(type) {
		case *bronAddress:
			e := &addrEntry{
				addr: *bronAddr,
			}
			copy(e.pubKeyHash160[:], bronAddr.AddrHash())
			if bronAddr.Imported() {
				// No order for imported addresses.
				importedAddrs = append(importedAddrs, e)
			} else if bronAddr.chainIndex >= 0 {
				// Chained addresses are sorted.  This is
				// kind of nice but probably isn't necessary.
				chainedAddrs[bronAddr.chainIndex] = e
			}
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], bronAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*bronAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only bronAddresses curently have private keys.
		a, ok := wa.(*bronAddress)
		if !ok {
			continue
		}
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (bronutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (bronutil.Address, error) {
	addr, err := s.nextChainedBronAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ChangeAddress(bs *BlockStamp) (bronutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedBronAddress(bs *BlockStamp) (*bronAddress, error) {
...
func (s *Store) LastChainedAddress() bronutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	newAddr, err := newBronAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newBronAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*bronAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a bronutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	// Look up address by address hash.
	bronaddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
func (s *Store) SetSyncStatus(a bronutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *bronutil.WIF, bs *BlockStamp) (bronutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := bronutil.Hash160(wif.SerializePubKey())
	...
	bronaddr, err := newBronAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	bronaddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		bronaddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = bronaddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := bronaddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = bronaddr
	s.importedAddrs = append(s.importedAddrs, bronaddr)
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (bronutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(bronutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*bronAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a bronAddress if !imported.
			bronAddr := addr.(*bronAddress)

			ws.chainIdxMap[bronAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
type WalletAddress interface {
	// Address returns a bronutil.Address for the backing address.
	Address() bronutil.Address
...
func (s *Store) ActiveAddresses() map[bronutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[bronutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the bronutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]bronutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// bronwallet shall refuse to open any unencrypted addresses.  This
...
// that bronwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*bronec.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*bronutil.WIF, error)
}

// newBronAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newBronAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *bronAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newBronAddressWithoutPrivkey(wallet,
...
// newBronAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newBronAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *bronAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case bronec.PubKeyBytesLenCompressed:
		compressed = true
	...
	address, err := bronutil.NewAddressPubKeyHash(bronutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
// newRootBronAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootBronAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *bronAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new bronAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newBronAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked bronAddress.
func (a *bronAddress) verifyKeypairs() error {
...
func (a *bronAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
	...
	addr, err := bronutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *bronAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *bronAddress) encrypt(key []byte) error {
...
func (a *bronAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	x, y := bronec.S256().ScalarBaseMult(privkey)
...
func (a *bronAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *bronAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *bronAddress) PubKey() *bronec.PublicKey {
	return a.pubKey
}

func (a *bronAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *bronAddress) ExportPubKey() string {
...
func (a *bronAddress) PrivKey() (*bronec.PrivateKey, error) {
...
func (a *bronAddress) ExportPrivKey() (*bronutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := bronutil.NewWIF((*bronec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
// normal key store.
func (a *bronAddress) watchingCopy(s *Store) walletAddress {
	return &bronAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *bronAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	address, err := bronutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := bronutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a bronutil.AddressScriptHash for a bronAddress.
func (sa *scriptAddress) Address() bronutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []bronutil.Address {
	return sa.addresses
}
