...
// brond RPC chain server, or an SPV library, as long as we write a driver for
// it.
type Interface interface {
	...
	Rescan(*chainhash.Hash, []bronutil.Address, map[wire.OutPoint]bronutil.Address) error
	NotifyReceived([]bronutil.Address) error
	NotifyBlocks() error
...
type (
	...
	FilterBlocksRequest struct {
		...
		ExternalAddrs    map[waddrmgr.ScopedIndex]bronutil.Address
		InternalAddrs    map[waddrmgr.ScopedIndex]bronutil.Address
		WatchedOutPoints map[wire.OutPoint]bronutil.Address
	}
	...
	FilterBlocksResponse struct {
		...
		FoundOutPoints     map[wire.OutPoint]bronutil.Address
