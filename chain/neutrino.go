...
func (s *NeutrinoClient) GetBlock(hash *chainhash.Hash) (*wire.MsgBlock, error) {
	...
	//  Should the block cache be INSIDE neutrino instead of in bronwallet?
...
func (s *NeutrinoClient) Rescan(startHash *chainhash.Hash, addrs []bronutil.Address,
	outPoints map[wire.OutPoint]bronutil.Address) error {
...
func (s *NeutrinoClient) NotifyBlocks() error {
	...
	if !s.scanning {
		s.clientMtx.Unlock()
		return s.NotifyReceived([]bronutil.Address{})
	}
	s.clientMtx.Unlock()
	return nil
}
...
func (s *NeutrinoClient) NotifyReceived(addrs []bronutil.Address) error {
	s.clientMtx.Lock()
...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, relevantTxs []*bronutil.Tx) {
