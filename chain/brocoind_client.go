...
func (c *BrocoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*bronjson.GetBlockVerboseResult, error) {
...
func (c *BrocoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*bronjson.GetBlockHeaderVerboseResult, error) {
...
func (c *BrocoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*bronjson.TxRawResult, error) {
...
func (c *BrocoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*bronjson.GetTxOutResult, error) {
...
func (c *BrocoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []bronutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]bronutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []bronjson.BlockDetails.
func (c *BrocoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]bronjson.RescannedBlock, error) {

	rescannedBlocks := make([]bronjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := bronjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *BrocoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]bronutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
func (c *BrocoindClient) rescan(start chainhash.Hash) error {
	...
	for i := previousHeader.Height + 1; i <= bestBlock.Height; i++ {
			...
			if headers.Back() != nil {
				...
				if headers.Back() != nil {
					previousHeader = headers.Back().
						Value.(*bronjson.GetBlockHeaderVerboseResult)
		...
		previousHeader = &bronjson.GetBlockHeaderVerboseResult{
