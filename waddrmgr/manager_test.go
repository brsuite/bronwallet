...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := bronutil.NewAddressPubKeyHash(
				pkHash, chainParams,
			)
			if err != nil {
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := bronutil.DecodeWIF(test.in)
			if err != nil {
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := bronutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext) bool {
	...
	for i, test := range tests {
		...
		switch test.typ {
		case addrPubKeyHash:
			addr, err = bronutil.NewAddressPubKeyHash(addrHash, chainParams)
		case addrScriptHash:
			addr, err = bronutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testLookupAccount(tc *testContext) bool {
	...
	for i, addr := range expectedAddrs {
		addr, err := bronutil.NewAddressPubKeyHash(addr.addressHash,
...
func TestScopedKeyManagerManagement(t *testing.T) {
	...
	_, ok := externalAddr[0].Address().(*bronutil.AddressScriptHash)
	if !ok {
	...
	_, ok = internalAddr[0].Address().(*bronutil.AddressWitnessPubKeyHash)
	if !ok {
		t.Fatalf("wrong type: %T", externalAddr[0].Address())
	}
