...
import (
	...
	"github.com/brsuite/bronwallet/internal/zero"
...
// Address returns the bronutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() bronutil.Address {
	return a.address
}
...
func (a *managedAddress) PubKey() *bronec.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*bronec.PrivateKey, error) {
	...
	privKey, _ := bronec.PrivKeyFromBytes(bronec.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*bronutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return bronutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *bronec.PublicKey, compressed bool,
	...
	if compressed {
		...
		pubKeyHash = bronutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address bronutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := bronutil.NewAddressWitnessPubKeyHash(
		...
		address, err = bronutil.NewAddressScriptHash(
	...
	case WitnessPubKey:
		address, err = bronutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *bronec.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*bronec.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *bronutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() bronutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := bronutil.NewAddressScriptHashFromHash(
